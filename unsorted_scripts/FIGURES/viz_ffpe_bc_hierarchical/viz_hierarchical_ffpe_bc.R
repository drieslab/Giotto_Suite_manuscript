...
# skipping highly variable genes detection. There are only 500 genes
...
screePlot(g, spat_unit = 'nucleus', ncp = 50) # try 20
...
giotto_leiden_clus = function(spat_unit, res) {
  g |>
    createNearestNetwork(spat_unit = spat_unit,
                         dimensions_to_use = dims_to_use[[spat_unit]]) |>
    doLeidenCluster(spat_unit = spat_unit,
                    resolution = res,
                    n_iterations = 100)
}

# same resolution used for leiden
g = giotto_leiden_clus('cell', res = 0.5)
g = giotto_leiden_clus('nucleus', res = 0.5)

# color codes based on sankey plot where a comparison between leiden clustering
# is made between the cell and nucleus spat_units
cell_color_codes = c(
  '#3485bc', '#b8cbea', '#f88c32', '#f9c288', '#37a941', '#a2e297'
)
nuc_color_codes = c(
  '#db3a40', '#faa29f', '#9d77c4', '#cbb8d8',
  '#97675a', '#cba69f', '#df84c3', '#fff299'
)
...
# visualize enrichment for clus 3 in cell spat_unit
spatPlot2D(
  gobject = g,
  spat_unit = 'cell',
...
prop_table = getSpatialEnrichment(g, spat_unit = 'cell', name = 'proportion', output = 'data.table')
prop_matrix = dt_to_matrix(prop_table)

# With the proportions information, use kmeans to classify each cell by their
# local leiden cluster proportions
...
sankeyPlot(
  sankey_dt,
  focus_names = c(paste0('domain_', c(3,5,8,9)),
...
domain_sub = domain[paste0('domain_', c(3,5,8,9))]
