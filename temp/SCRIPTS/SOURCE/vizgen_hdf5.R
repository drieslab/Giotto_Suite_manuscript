...
# 
# Moving forward, these functions should not be necessary and a simpler workflow
...
## vizgen image creation ####
#
# load image and apply transforms to turn it into microns
#
# produces giottoLargeImage with spatial extent that matches that of the HDF5
# polygon bounds information
create_vizgen_large_image = function(image_file,
                                     transforms_file,
                                     name = 'image') {
  tfsDT = data.table::fread(transforms_file)
  gimg = Giotto::createGiottoLargeImage(image_file,
                                        name = name,
                                        scale_factor = c(1/tfsDT[[1,1]], 1/tfsDT[[2,2]]),
                                        negative_y = FALSE)
  gimg = spatShift(gimg,
                   dx = -tfsDT[[1,3]]/tfsDT[[1,1]],
                   dy = -tfsDT[[2,3]]/tfsDT[[2,2]])
...
# 
# NOTE: SpatRaster input is required (the image created with create_vizgen_large_image)
# This is so that spatial flipping can take the entire dataset's spatial bounds
# into account.
ext_query_polys = function(vizproxy, SpatRaster, poly_crop_ext) {
