...
for (segmentation in c("cell", "StarDist", "mesmer", "CellPose", "Baysor")) {
    spatInSituPlotPoints(xen_ROI,
        ...
        use_overlap = FALSE,
        save_param = list(
            save_name = sprintf("S8A_%s", segmentation),
            save_format = "pdf"
        )
    )
}
...
ann = c("B", "pDC")
names(ann) <- seq_along(ann)
subclus_bdc <- annotateGiotto(subclus_bdc, annotation_vector = ann, 
...
names(ann) <- seq_along(ann)
subclus_dcis <- annotateGiotto(subclus_dcis, annotation_vector = ann, 
...
names(ann) <- seq_along(ann)
subclus_stromal <- annotateGiotto(subclus_stromal, annotation_vector = ann, 
                                  cluster_column = "leiden_clus", name = "subclus_stromal")
subclus_stromal_res <- spatValues(subclus_stromal, feats = "subclus_stromal")
join_xen <- addCellMetadata(join_xen, new_metadata = subclus_stromal_res, by_column = TRUE)
...
ggplot2::ggsave(file.path(results_folder, "S8D_area_violin.pdf"), violin_area)
...
ggplot2::ggsave(
    filename = file.path(results_folder, "S8E_celltype_comp.pdf"), 
    plot = ct_comp,
    dpi = 300,
    units = "in",
    height = 7,
...
ggplot2::ggsave(
    filename = file.path(results_folder, "S8F_celltype_stack.pdf"), 
    plot = ct_stack
)
