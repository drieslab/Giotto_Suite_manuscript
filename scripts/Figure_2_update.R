...
eplot_l <- enrichplot::dotplot(gsea_inside_outside, showCategory = 6, split=".sign") + 
...
##### Fig 2 M ---------------------------------------------------------- ####
eplot_m <- enrichplot::dotplot(gsea_nuc_cyto, showCategory = 6, split = ".sign") + 
  enrichplot::facet_grid(.~.sign)
ggplot2::ggsave(
    plot = eplot_m,
...
##### Fig 2I ---------------------------------------------------------- ####
svg(file.path(fig_dir, "I.svg"), height = 7, width = 9)
plot(cell_poly, background = 'black', col = 'darkgreen', alpha = 0.7,)
