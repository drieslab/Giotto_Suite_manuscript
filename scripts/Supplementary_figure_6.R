...
instructions(g, "save_dir") <- "scripts/FIGURE/S6/"
...
plot(g@spatial_info$aggregate, add = TRUE)
plot(g@spatial_info$aggregate, type = "centroid", 
     add = TRUE, cex = 0.5)
...
pl
# with Gad1 label
pl <- pl + geom_point(data = spatial_diff[genes == "Gad1"], 
                      aes(x = log(sumt + 1), y = log(covt + 1), 
                          color = "red")) +
  geom_text(data = spatial_diff[genes == "Gad1"], 
            aes(x = log(sumt + 1), y = log(covt + 1), 
                color = "red"), 
            label = "Gad1",
            hjust = -0.2)
