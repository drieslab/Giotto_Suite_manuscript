...
### calculate overlap for original (cell) and rescaled (smallcell) polygons ####
...
############### Normalize data using simple pearson residuals ##################
...
testg <- createNearestNetwork(testg,
                              spat_unit = "smallcell",
                              dimensions_to_use = pcs)
...
### Compare k-means clustering results for spatial units cell and smallcell ####

library(data.table)

# combine metadata from spatial units 'cell' and 'smallcell'
small_cell_meta <- pDataDT(testg,
                           spat_unit = "smallcell")

setnames(small_cell_meta, old = "kmeans", new = "km_smallcell")

cell_meta <- pDataDT(testg,
                     spat_unit = "cell")
...
testg <- addCellMetadata(testg,
    new_metadata = comb_meta_stable,
    by_column = TRUE,
...
################## Identify spatial network neighbors ##########################
...
pl <- ggplot()
pl <- pl + geom_bar(
    data = cell_metadata, aes(x = "", fill = km_stable),
    position = "fill"
)
...
pl <- ggplot()
pl <- pl + geom_bar(
    data = cell_metadata, aes(x = "", fill = nb_cells),
    position = "fill"
)
...
prop_table <- getSpatialEnrichment(testg,
                                   name = "km_niche",
                                   output = "data.table")
