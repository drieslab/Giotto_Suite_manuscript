...
baysor_dt <- data.table::fread("./Xenium/Aligned_Xe_rep1/Baysor_polygons_aligned.csv",
    drop = c(1, 2, 3, 4)
) # drop index, old x, and old y columns
...
baysor_gpoly <- giottoPolygon(
    spatVector = baysor_poly,
...
if (REMAKE_JOINT_OBJECT) {
    ...
    tictoc::tic()
    sub_join_xen <- doLeidenClusterIgraph(
        gobject = sub_join_xen,
        spat_unit = "cell",
        feat_type = "rna",
    ...
    join_xen <- doClusterProjection(
        target_gobject = join_xen,
...
names(leiden_clusters) <- c(
    ct_c1, ct_c2, ct_c3, ct_c4, ct_c5, ct_c6, ct_c7,
...
ggplot2::ggsave(
    filename = "s8E_percentage_cell_type_by_segmentation.png",
    plot = last_plot(),
    device = "png",
    width = 14,
    height = 6,
...
### Bar Plot (stacked)
ggplot2::ggplot(dt_ct, aes(
    x = factor(cell_type, levels = dec_order),
...
ggplot2::ggsave(
    filename = "s8C_UMAPs_each_segmentation.pdf",
    plot = both_umaps,
    device = "pdf",
    width = 40,
    height = 20
...
ggplot2::ggsave(
    filename = "s8C_UMAPs_each_segmentation.png",
    plot = both_umaps,
    device = "png",
    width = 30,
    height = 15,
...
ggplot2::ggsave(
    filename = "s8B_spatPlot_cell_types_each_segmentation.pdf",
    plot = both_spatPlots,
    device = "pdf",
    width = 40,
    height = 15
...
ggplot2::ggsave(
    filename = "s8B_spatPlot_cell_types_each_segmentation.png",
    plot = both_spatPlots,
    device = "png",
    width = 30,
    height = 15
...
vi_pl <- ggpubr::ggviolin(polygonal_areas,
    x = "segmentation", y = "polygonal_area", fill = "segmentation",
    ...
    ggpubr::stat_compare_means(comparisons = list(c("Original", "Baysor")),
