...
sd_nuc_gpoly <- giottoPolygon(
    spatVector = sd_nuc_poly,
...
lin_model <- lm(
    formula = dt_ERBB2 ~ dt_HER2,
    data = as.data.frame(dt[, .(ERBB2, HER2)])
)

plot(
    x = values(final_combined_rast$HER2),
...
lin_model_2 <- lm(
    formula = dt2_MS4A1 ~ dt2_CD20,
    data = as.data.frame(dt2[, .(MS4A1, CD20)])
)

plot(
    x = terra::values(final_combined_rast_2$CD20),
...
her2_glimg_cropped <- giottoLargeImage(
    name = "HER2",
...
terra::plot(her2_glimg_cropped@raster_object,
    col = GiottoVisuals::getColors(pal = "blues", rev = TRUE)[50:100]
)
terra::plot(roi3@spatial_info$cell@spatVector,
            lwd = 0.5,
            add = TRUE
)
terra::plot(cropped_ERBB2_vect,
            col = "red",
            cex = 0.5,
            alpha = 0.6,
            add = TRUE)
